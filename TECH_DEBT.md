...
This project is in strong shape. Eight structural suites (domain purity, layer boundaries, module size, style, a no-network invariant, the donate button's address and seam, plus the two halves of the focus-ring invariant) share one AST scanner, the no-network invariant is proven over everything a user installs rather than over the package alone, the size cap covers the test tree and carries a danger band, the package and the setup program's Qt-free halves are gated at 100% line and branch coverage, `VERSION` is the single source of truth (nothing outside it names the product's version; the version-like literals in `tests/installer/` are fixture data chosen to exercise the upgrade comparison) and there is not a single em dash in the tree. One item remains and it is continuous rather than a task with an end state.
...
- The seventeen tracked PNGs plus the `.ico` and `.icns`. Emitted by `generate_icons.py` from two masters and consumed by named packaging paths on three platforms. Three of the PNGs are the donation mark: its master, the copy the application bundles and the copy the site serves, written by one render so they cannot drift.
...
- `tests/structural/scan.py` being a helper module inside the test tree rather than a package. One scanner shared by eight suites is exactly right, as is the small stylesheet reader `focus_scan.py` beside it.
- Twenty-six findings from a default `ruff check`, measured, none of them in the gate this repository actually runs (black and flake8, both asserted in-suite by `test_style.py`). They are import ordering in `main.py`, `Mapping` imported from `typing` rather than `collections.abc`, `timezone.utc` rather than the `datetime.UTC` alias and `subprocess.run` without an explicit `check` in two delivery scripts. Discretionary modernisation rather than debt. Three of them would be actively wrong to accept. `RUF100` calls the `# noqa: N802 (Qt override)` comments on `event()` and `showEvent()` unused, which is true only because no pep8-naming plugin is installed; the comment is what tells the next reader the odd casing is Qt's and not a slip. It says the same of the `# noqa: E402` in `tests/conftest.py`, where removing the comment was measured to fail flake8 outright: the import genuinely sits after the line that selects the offscreen platform, where it has to be.

## Not debt (do not "fix" these)

These look like candidates but are correct as they stand; changing them would regress or add cost for nothing.

- **`test_no_network.py` itself.** It is the mechanism that makes "your games never touch the network" a fact rather than a marketing line; it is the single best-targeted structural test in the portfolio. It proves the claim over everything a user installs (the package, both composition roots and the whole setup program) and its own scope is asserted alongside it, so narrowing it back to the package fails the suite rather than passing quietly. The update check's GitHub adapter is its single named exemption, asserted whole: the module must ship, must import `urllib.request` and may import nothing forbidden beyond it, so the hole can neither widen nor outlive its purpose.
...
- **The wire format staying at v1 while the application's own version moves.** They are separate numbers answering separate questions and they are not meant to track each other. `VERSION` describes the application; the token in the BEGIN line describes the contract two installations share and moves only when that contract breaks. A game that has been running for months is the reason: the players will not be on the same application version, so the block has to mean the same thing to both of them. Do not "align" them.
- **The eight separate structural suites** rather than one file of assertions. Each fails with a message about one invariant, which is what makes a red suite diagnostic rather than merely red.
